...
class URL_Case(db.Model):
    ...
    urlscan_result = db.Column(db.Text, default="{}")
    abuseipdb_result = db.Column(db.Text, default="{}")
    static_analysis = db.Column(db.Text, default="{}")
    screenshot_path = db.Column(db.String(500), nullable=True)
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))
    updated_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc), onupdate=lambda: datetime.now(timezone.utc))
    completed_at = db.Column(db.DateTime, nullable=True)
    indicators = db.relationship("Indicator", backref="case", lazy="dynamic", cascade="all, delete-orphan")
    ...
    def __repr__(self):
        return f"<URL_Case {self.case_id[:8]} [{self.status}] score={self.risk_score}>"
